# I will import the tidyverse library 
library(tidyverse)
...
# =============================================================================
# STEP 3: CLEANING MY DATASETS TO MERGE THEM
# =============================================================================

# After taking a look at the datasets: the 2019 datasets have the same columns, Q1 has different names,
# so names will have to be changed for all 4 datasets to have the same names. 
# 1. Names will have to be changed in order for all 4 tables to have the same column names
# 2. Characters will have to be changed: id to chr for 2019 datasets (2020 ds id columns cannot be changed 
# to int), dates in every tables will be changed to datetime, tripduration will be changed to int
# 3. New columns: 2020 tables will have new column for trip duration, columns will be reordered
# 4. Delete columns - in 2019 tables: bike_id (there is no bike_id in 2020), gender, birthyear
#     - in 2020 table: rideable_type, start_lat, start_lng, end_lat, end_lng
# 5. 2019 datasets have 'Customer' or 'Subscriber', we will change it to 2020 values 
# 'Casual' or 'Member'
...
# =============================================================================
# STEP 4: MERGE THE DATASETS AND CLEAN THEM
# =============================================================================
...
dim(trips_df_v2)
# Lastly, I will fix the only null value there is.
# Since it it a row without info, it will be deleted
View(filter(trips_df_v2, is.na(end_station_id)))
...
# =============================================================================
# STEP 5: PERFORM ANALYSIS ON THE DATASET
# =============================================================================
View(trips_df_v3)
...
counts <- aggregate(trips_df_v3$tripduration ~ trips_df_v3$usertype +
                      trips_df_v3$day_of_week, FUN = mean)
counts
